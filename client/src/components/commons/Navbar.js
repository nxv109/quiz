...
export default function Navbar(){
    ...
    React.useLayoutEffect(() => {
        setInfoUser(JSON.parse(sessionStorage.getItem('info_user_s')) || JSON.parse(localStorage.getItem('info_user_l')));
    }, []);
    ...
    return(
        <nav className="navbar navbar-expand-lg navbar-light bg-light">
            <Link className="nav-link" to="/"><img style={{ width: "50px" }} src="https://img.icons8.com/plasticine/100/000000/classroom.png" alt="icon_logo" /></Link>
            ...
            <ul className="navbar-nav mr-auto">
                ...
                <li className="nav-item dropdown">
                    <a aria-expanded="false" aria-haspopup="true" className="nav-link dropdown-toggle" data-toggle="dropdown" href="#0" id="navbarDropdown" role="button">
                        { infoUser ? (<img style={{width: "25px"}} src="https://img.icons8.com/plasticine/100/000000/user-male-circle.png" alt="" />) : "Account" }
                    </a>
                    {
                        infoUser ?
